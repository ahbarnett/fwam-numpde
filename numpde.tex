...
Here, usually user gets to choose the nodes $x_j$ \hfill\com{so pick best ones!}
...
% ---------------------------------------------------
\begin{frame}\ft{Finite difference idea}

  set up linear system for 1D ODE $u'' = g$ w/ some BCs
  
  Uniform 1D grid in $[0,1]$  \quad $h=1/N$

  $u''(x) = \frac{u(x-h)-2u(x)+u(x+h)}{h^2} + \bigO(h^4)$  \com{ via Taylor thm}

show results as $h\to 0$ for known $u$
...
% ---------------------------------------------------
\begin{frame}\ft{Timestepping with finite differente methods}
