...
\quad \com{ie error controlled by mass of tail}
...
\gb local piecewise linear $\rightarrow$ composite trapezoid rule
   ...
   \qquad \com{$w_j=h/2$ at ends, $h$ in middle. \; low-order: error $\bigO(N^{-2})$}
   \emp
   \hfill
   \pig{0.9in}{figs/trap}

\gb global poly $\rightarrow$ Newton--Coates

\gb can do better: is ``Gaussian'' node set giving twice this rate!

$x_j,w_j$ codes in scipy, matlab, etc

Periodic case:

  periodic spectral get periodic trap rule $\bigO(c^{-N})$ if analytic

  all $w_j = \frac{2\pi}{N}$
  
  PTR exact out to $N-1$, not merely $N/2$  (detail)

  Adaptive panels each with Gaussian 
...
custom quadratures for singularities, eg $f(x) = \mbox{smooth}\cdot|x|^{1/2}$
 \who{CCM: ask Manas Rachh}

  change of variables (sinh-tanh transform to remove endpoint singularities)
  
  Higher dims: For $d$ up to a few, products of 1D nodes
  \com{exp.\ cost $N=n^d$}
  
  \com{can also do in other coord sys, eg sphere = $z \in [-1,1]$ tensor prod.\ with periodic $\phi\in[0,2\pi)$}

    \com{adaptivity works: subdivide boxes}
...
% ---------------------------------------------------
\begin{frame}\ft{Recap concepts}


  repeat list of concepts

  See refs at end
  
  
%extrapolation: possible if know $f$ is smooth
